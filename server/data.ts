...
export function can_create_class(e: Email): boolean {
  return (
    env.config_class_creators.includes('*') || // Match any
    env.config_class_creators.includes(`*@${e.split('@')[1]}`) || // Match email domain
    env.config_class_creators
      .filter((p) => p.includes('/'))
      .some((p) => new RegExp(p, 'g').test(e)) || // Match regex
    env.config_class_creators.includes(e)
...
export function validate_class(c: Class): boolean {
  return (
    ...
    Object.entries(c.members).every((e) =>
      Object.values(RoleName).includes(e[0] as RoleName)
    ) &&
    Object.entries(c.members).every((e) =>
      e[1].every((v, _i, _a) => validate_email(v))
    ) &&
